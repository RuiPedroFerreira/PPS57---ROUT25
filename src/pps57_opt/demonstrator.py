...
def render_demonstrator_markdown(report: JsonDict) -> str:
    ...
    lines.extend(
        _counter_section("Controller rejects", runtime.get("controller_rejection_by_reason", {}))
    )
    ...
    if not counterfactuals.get("available"):
        ...
        lines.append("")
        lines.extend(
            _counter_section("Runtime by action", counterfactuals.get("runtime_by_action", {}))
        )
        lines.extend(
            _counter_section(
                "Offline baseline by action", counterfactuals.get("offline_baseline_by_action", {})
            )
        )
        lines.extend(
            _counter_section(
                "Offline selected by action", counterfactuals.get("offline_selected_by_action", {})
            )
        )
...
def _offline_counterfactuals(tsp_payload: JsonDict, policy_summary: JsonDict | None) -> JsonDict:
    ...
    return {
        ...
        "offline_action_changes_vs_baseline": policy_summary.get(
            "optimized_action_changes_vs_baseline"
        ),
        "offline_action_unchanged_vs_baseline": policy_summary.get(
            "optimized_action_unchanged_vs_baseline"
        ),
...
def _run_payload(run: DemonstratorRun) -> JsonDict:
    ...
    runtime = {
        ...
        "applied_events": _summary_or_count(
            run.summary, "applied_events", [a for a in run.actuations if a.get("applied")]
        ),
        ...
        "controller_rejection_by_reason": dict(
            Counter(_controller_rejection_reason(item) for item in controller_rejections)
        ),
...
def _limitations(*runs: DemonstratorRun) -> list[str]:
    ...
    if not any(d.get("status") == "blocked_by_safety" for run in runs for d in run.decisions):
        limitations.append(
            "No Safety Layer block was observed in the provided runs; safety paths may require stress scenarios to appear in runtime evidence."
        )
    return limitations
...
def _per_tls(
    ...
    for tls_id in sorted(tls_ids):
        payload[tls_id] = {
            ...
            "applied_events": sum(
                1 for item in actuations if item.get("tls_id") == tls_id and item.get("applied")
            ),
            "safety_blocks": sum(
                1
                for item in decisions
                if item.get("tls_id") == tls_id and item.get("status") == "blocked_by_safety"
            ),
            "controller_rejections": sum(
                1 for item in controller_rejections if item.get("tls_id") == tls_id
            ),
        }
    return payload
